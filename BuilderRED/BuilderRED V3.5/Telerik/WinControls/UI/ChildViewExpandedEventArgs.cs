// Decompiled with JetBrains decompiler
// Type: Telerik.WinControls.UI.ChildViewExpandedEventArgs
// Assembly: BuilderRED, Version=3.5.0.0, Culture=neutral, PublicKeyToken=null
// MVID: C1B998B6-FDC9-4BE9-BF3E-DE300E192916
// Assembly location: C:\Program Files (x86)\ERDC-CERL\SMS BUILDER RED\BuilderRED.exe

using System;

namespace Telerik.WinControls.UI
{
  public class ChildViewExpandedEventArgs : EventArgs
  {
    private GridViewHierarchyRowInfo parentRow;

    public ChildViewExpandedEventArgs(GridViewHierarchyRowInfo parentRow)
    {
      this.parentRow = parentRow;
    }

    public GridViewRowInfo ParentRow
    {
      get
      {
        return (GridViewRowInfo) this.parentRow;
      }
    }

    public GridViewDetailsRowInfo ChildRow
    {
      get
      {
        return this.parentRow.ChildRow;
      }
    }

    public GridViewInfo ChildViewInfo
    {
      get
      {
        return this.parentRow.ActiveView;
      }
    }

    public bool IsExpanded
    {
      get
      {
        return this.parentRow.IsExpanded;
      }
    }
  }
}
