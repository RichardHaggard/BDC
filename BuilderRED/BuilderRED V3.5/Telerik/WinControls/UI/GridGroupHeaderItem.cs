// Decompiled with JetBrains decompiler
// Type: Telerik.WinControls.UI.GridGroupHeaderItem
// Assembly: BuilderRED, Version=3.5.0.0, Culture=neutral, PublicKeyToken=null
// MVID: C1B998B6-FDC9-4BE9-BF3E-DE300E192916
// Assembly location: C:\Program Files (x86)\ERDC-CERL\SMS BUILDER RED\BuilderRED.exe

namespace Telerik.WinControls.UI
{
  public class GridGroupHeaderItem : LightVisualElement
  {
    private GridGroupByField field;
    private GridViewDataColumn column;

    public GridGroupHeaderItem(GridGroupByField field, GridViewDataColumn column)
    {
      this.field = field;
      this.column = column;
    }

    public GridGroupByField GroupByField
    {
      get
      {
        return this.field;
      }
    }

    public GridViewDataColumn Column
    {
      get
      {
        return this.column;
      }
    }

    public GridViewTemplate ViewTemplate
    {
      get
      {
        return this.column.OwnerTemplate;
      }
    }
  }
}
