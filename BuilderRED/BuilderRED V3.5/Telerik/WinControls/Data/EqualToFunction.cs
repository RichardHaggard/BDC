...
// Type: Telerik.WinControls.Data.EqualToFunction
...
{
  internal class EqualToFunction : FilterFunction
  {
    ...
    public EqualToFunction()
    {
      ...
      this.gkfunction = GridKnownFunction.EqualTo;
    }
    ...
    public override List<Type> DefaultForTypes
    {
      get
      {
        if (this.defaultForTypes == null)
          this.defaultForTypes = new List<Type>((IEnumerable<Type>) new Type[6]
          {
            typeof (short),
            typeof (int),
            typeof (double),
    ...
    public override List<Type> NotApplicableForTypes
    {
      get
      {
        if (this.notApllicableForTypes == null)
          this.notApllicableForTypes = new List<Type>((IEnumerable<Type>) new Type[2]
          {
            typeof (byte[]),
            typeof (Image)
          });
    ...
    public override string DisplayName
    {
      get
      {
        return LocalizationProvider<RadGridLocalizationProvider>.CurrentProvider.GetLocalizedString("FilterFunctionEqualTo");
      }
    }
  }
}
