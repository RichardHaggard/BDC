...
// Type: Telerik.Collections.Generic.ListIndex`1
...
{
  public class ListIndex<T> : Index<T> where T : IDataItem
  {
        ...
        foreach (T obj in objs)
        {
          if (this.CollectionView.PassesFilter(obj))
          {
            if (flag)
            {
              ...
              int num = items.BinarySearch(obj, this.CollectionView.Comparer);
              if (num < 0)
                items.Insert(num * -1 - 1, obj);
            }
            else
              this.items.Add(obj);
          }
